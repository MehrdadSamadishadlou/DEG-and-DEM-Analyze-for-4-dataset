...
mirset <- readRDS('Healthy_CAD_4dataset_microRNA.Rds')
exp <- exprs(mirset)
...
write.csv(CAD.up, file="Results/just microRNA/CAD_upper.csv", quote = F, row.names = F, col.names = F)
...
write.csv(DEGs, file="Results/DEGs_CAD_Healthy_mirs.csv", quote = F, row.names = F, col.names = F)
